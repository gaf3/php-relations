...
class Relations_Admin_Date extends Relations_Admin_Input {
  ...
  function entered($records) {
    ...
    for ($record = 0; $record < $records; $record++) {
      ...
      // If nothings sent in part, assume single

      if (!$year && !$month && !$day)
        list($year,$month,$day) = explode('-',Relations_Admin_grab($this->prefix . $this->name . '_' . $record,$this->values[$record],'VPG'));
      ...
      $this->values[$record] = "$year-$month-$day";
  ...
  function massed($records) {
    ...
    if (Relations_Admin_grab($this->prefix . $this->name . '_mass',false,'VPG')) {
      ...
      if (!$year && !$month && !$day)
        list($year,$month,$day) = explode('-',Relations_Admin_grab($this->prefix . $this->name,'','VPG'));
  ...
  function enterHTML($record,$alive) {
    ...
    return Relations_Admin_MessageHTML($this,$errors,'error') . 
           ...
           Relations_Admin_DateHTML($this,$name,$this->values[$record],false);
  ...
  function massHTML() {
    ...
    return Relations_Admin_CheckboxHTML($this,$name . '_mass',1,'Use for Mass',false,'mass_') . "<br>\n" .
           Relations_Admin_DateHTML($this,$name,$this->values[0],false,'',$changed);
  ...
  function searchHTML() {
    ...
    return Relations_Admin_CheckboxHTML($this,$name . '_search',1,'Use in Search',$this->sought,'search_') . "<br>\n" .
           Relations_Admin_DateHTML($this,$name,$this->values[0],true,'',$changed);
