...
class Relations_Admin_ChildMM extends Relations_Admin_ChildDB {
  ...
  function Relations_Admin_ChildMM() {
  ...
  function childrenValidate(&$errors,$intentions,$records) {
    ...
    for ($record = 0; $record < $records; $record++)
      if ($intentions[$record] == 'delete')
        $ins[] = $this->form->childSQL($this->descendants,$this->mm_local_field,$this->local_originals[$record]);
    ...
    $where = $this->form->childWhere($this->descendants,$this->mm_local_field,$ins);

    // See if there's any records

    $parents = Relations_toHash($this->form->abstract->selectColumn(array(
      _field => $this->mm_local_field,
      _query => new Relations_Query(array(
        _select  => $this->mm_local_field,
  ...
  function labelDB(&$errors,&$totals,$intention,$ambition,$records) {
    ...
    if ($this->descendants == 'oprefix') {
      ...
      $ins = array();
      for ($record = 0; $record < $records; $record++)
        $ins[] = $this->form->childSQL($this->descendants,"$this->mm_table.$this->mm_local_field",$this->local_originals[$record]);
      ...
      $where = $this->form->childWhere($this->descendants,"$this->mm_table.$this->mm_local_field",$ins);
      ...
      if ($this->display == 'count')
        $set = array(_group_by => "$this->mm_table.$this->mm_local_field");
      else
        $set = array();
